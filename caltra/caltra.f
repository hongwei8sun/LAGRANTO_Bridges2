...
c     Meteorological fields
      ...
      real,allocatable, dimension (:)     :: temp0,temp1     ! 3d temperature !!! shw
...
C     Allocate memory for some meteorological arrays
      ...
      allocate(temp0(nx*ny*nz),stat=stat) !!! shw
      if (stat.ne.0) print*,'*** error allocating array temp0 ***'  ! temperature !!! shw
      allocate(temp1(nx*ny*nz),stat=stat) !!! shw
      if (stat.ne.0) print*,'*** error allocating array temp1 ***' !!! shw
...
c     Read wind fields and vertical grid from first file
     ...
     >     xmin,xmax,ymin,ymax,dx,dy,nx,ny,nz,timecheck)
      ...
      varname='T'                                      ! temp
      call input_wind
     >    (fid,varname,temp1,tload,stagz,mdv,
     >     xmin,xmax,ymin,ymax,dx,dy,nx,ny,nz,timecheck)
     !!! shw
...
!!! shw
        ...
        enddo !  do iloop=filo,lalo


!!! shw
        IF( real(deltout) > real(timeinc)*60.0 )THEN
        if( mod(nint(time1),nint(deltout/60.0))==0 )then
           time = time1
           itim = itim + 1
           IF( itim.le.ntim ) THEN
           do i=1,ntra
...
C     jump	 input  flag (=1 trajectories do not enter the ground)
...
C     Interpolate wind fields to starting position (x0,y0,p0)
     ...
     >                 nx,ny,nz,xmin,ymin,dx,dy,mdv)
      ...
      T_K0 =int_index4(temp0,temp1,nx,ny,nz,xind,yind,pind,reltpos0,mdv) !!! shw
...
C        Calculate new winds for advection
     ...
     >                    nx,ny,nz,xmin,ymin,dx,dy,mdv)
         ...
         T_K1=int_index4(temp0,temp1,nx,ny,nz,xind,yind,pind,
     >                   reltpos1,mdv) !!! shw
...
!------------------------------------------------------------------
         ...
         IF(Is_settling==1)THEN
          ...
          vis =10.0*1.8325e-5*(416.16/(T_K+120.0))*(T_K/296.16)**1.5

          ! Free mean path of air [cm]
          fpair = 6.6e-6*(1013./P_hPa)*(T_K/293.15)
	  ...
	  ! 5.0e-6cm   = 0.05 um
          ...
          vt =vt *
     >         ( 1.0+ 1.257*fpair/aer_ar_wet +
     >                  0.4*fpair/aer_ar_wet*
     >            EXP(-1.1/(fpair/aer_ar_wet)) )
...
C     Interpolate surface pressure to actual position
     ...
     >                 p3d0,p3d1,spt0,spt1,3,
     >                 nx,ny,nz,xmin,ymin,dx,dy,mdv)
      sp = int_index4 (spt0,spt1,nx,ny,1,xind,yind,1.0,reltpos,mdv)
